WITH base AS (
  ...
  WHERE NOT l.insect_relevant_date AND offset >= 1
),
...
with_roll AS (
  SELECT
    *,
    AVG(daily_mean) OVER (
      PARTITION BY bird, arrival_year, effective_year, location_name
...
SELECT
  with_roll.*,
  ROW_NUMBER() OVER (
    PARTITION BY bird, arrival_year, effective_year, location_name, chunk_id
    ORDER BY weather_date
  ) AS day_idx
FROM with_roll